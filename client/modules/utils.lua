...
function utils.addRemoveItem(type, item, quantity)
    ...
    TriggerServerEvent("dg_emsjob:removAddItem", data)
end
...
function utils.useItem(item, value)
    ...
    data.item = item
    data.value = value
    TriggerServerEvent("dg_emsjob:useItem", data)
end

function utils.getItem(name)
    local item = lib.callback.await('dg_emsjob:getItem', false, name)
...
RegisterNetEvent('dg_emsjob:showNotification', utils.showNotification)
