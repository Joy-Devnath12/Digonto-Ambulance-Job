...
local function checkInParamedic(ped, hospital)
    if inv.payForTreatment(Config.ParamedicTreatmentPrice) then
        ...
        -- Let player get up right at the bed
...
function initParamedic()
    for index, hospital in pairs(Config.Hospitals) do
        ...
        addLocalEntity(ped, {
            {
                label = locale('check_in') or "Check In",
                icon = 'fa-solid fa-notes-medical',
                groups = false,
                fn = function()
                    if not Config.AllowAlways then
                        local medicsOnline = lib.callback.await('dg_emsjob:getMedicsOniline', false)
                        if medicsOnline <= 0 then
                            checkInParamedic(ped, hospital)
                        else
