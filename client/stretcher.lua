...
function putOnStretcher(toggle, target)
    ...
    print(data.target)
    TriggerServerEvent("dg_emsjob:putOnStretcher", data)
    patientOnStretcher = toggle and target or nil
end

RegisterNetEvent("dg_emsjob:togglePatientFromVehicle", function(veh)
    local playerPed = cache.ped or PlayerPedId()
...
RegisterNetEvent("dg_emsjob:putOnStretcher", function(toggle)
    local playerPed = cache.ped or PlayerPedId()
    local playerCoords = cache.coords or GetEntityCoords(playerPed)
...
local function vehicleInteractions()
    local options = {
        ...
        {
            name = 'dg_emsjob_put_patient_in_vehicle',
            icon = 'fa-solid fa-car-side',
            ...
            fn = function(data)
                ...
                TriggerServerEvent('dg_emsjob:togglePatientFromVehicle', dataToSend)
        ...
        {
            name = 'dg_emsjob_take_patient_from_vehicle',
            icon = 'fa-solid fa-car-side',
...
local function stretcherInteraction()
    addModel({ `prop_ld_binbag_01` }, {
        {
            name = 'dg_emsjob_stretcher_model',
            label = locale('take_stretcher'),
        ...
        {
            name = 'dg_emsjob_remove_from_stretcher',
            icon = 'fa-solid fa-car-side',
