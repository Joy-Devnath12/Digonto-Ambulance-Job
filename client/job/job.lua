...
local function checkPatient(target)
    ...
    local data = lib.callback.await('dg_emsjob:getData', false, targetServerId)
    ...
    local options = {
        {
            title = ("%s: %s"):format(locale("status_patient"), status),
            icon = 'heartbeat',
    ...
    if isDead then
        options[#options + 1] = {
            ...
            iconColor = "#5BC0DE",
            onSelect = function()
                local count = inv.getItemCount('defibrillator')
                ...
                TriggerServerEvent("dg_emsjob:healPlayer", dataToSend)
            end,
        }
...
function createDistressCall(msg)
    ...
    if not msg then
        local input = lib.inputDialog('dg EMS', {
    ...
    if not Config.UseInterDistressSystem then
        ...
        TriggerServerEvent("dg_emsjob:createDistressCall", data)
    end
...
function openDistressCalls()
    ...
    local distressCalls = lib.callback.await('dg_emsjob:getDistressCalls', false)
                ...
                lib.registerContext({
                    ...
                    options = {
                        ...
                        },
                        {
                            ...
                            onSelect    = function()
                                TriggerServerEvent("dg_emsjob:callCompleted", call)
...
RegisterNetEvent('dg_emsjob:useEmsTablet', openDistressCalls)
...
RegisterNetEvent("dg_emsjob:playHealAnim", function(data)
    local playerPed = cache.ped or PlayerPedId()
...
RegisterNetEvent("dg_emsjob:createDistressCall", function(name)
    if not hasJob(Config.EmsJobs) then return end
